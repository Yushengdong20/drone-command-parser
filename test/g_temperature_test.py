...
def run_temperature_test():
    ...
    # 清理GPU信息，使其适合作为文件名
    gpu_info_clean = gpu_info.replace(' ', '_').replace(',', '_').replace(':', '_').replace('\\', '_').replace('/', '_')
    # 限制GPU信息长度
    gpu_info_clean = gpu_info_clean[:50]
    
    # 添加编号逻辑，确保文件名唯一
    base_filename = f'temperature_test_report_{gpu_info_clean}{report_suffix}'
    extension = '.md'
    counter = 1
    ...
    # 测试配置：只改变温度，其他参数不变
    # 基准配置：qwen3.5:latest + 高上下文（16384）
    test_configs = [
        {
            "name": "温度 0.0",
            "temperature": 0.0
    ...
    # 生成报告
    generate_report(test_results, test_logs, report_filename, test_cases, gpu_info)


def generate_report(test_results, test_logs, report_filename, test_cases, gpu_info):
    """
    生成性能测试报告
    ...
    # 找出所有最高准确率的配置
    max_accuracy = max(r['match_rate'] for r in all_results)
    best_accuracy_results = [r for r in all_results if r['match_rate'] == max_accuracy]
    ...
    for log in test_logs:
        # 只匹配类似"=== 测试配置: xxx ==="的模式
        if log.startswith("===") and "测试配置" in log:
            current_config = log
            config_logs[current_config] = []
        elif current_config:
            config_logs[current_config].append(log)
