...
def run_temperature_test():
    ...
    report_filename = os.path.join(report_dir, f'temperature_test_report{report_suffix}.md')
    ...
    # 生成报告
    generate_report(test_results, test_logs, report_filename, test_cases)


def generate_report(test_results, test_logs, report_filename, test_cases):
    """
    生成性能测试报告
    ...
    best_accuracy_result = max(all_results, key=lambda x: x['match_rate'])
    ...
    report += f"**最高准确率配置**: {best_accuracy_result['name']}\n"
    report += f"**匹配率**: {best_accuracy_result['match_rate']:.2f}%\n\n"
    ...
    print(f"最高准确率配置: {best_accuracy_result['name']}")
    print(f"匹配率: {best_accuracy_result['match_rate']:.2f}%")
